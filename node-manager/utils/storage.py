...
class Storage(object):
    ...
    def make_logical_volume(self,vg,name,size,owner):
        if name not in self.all_lv :
            comm = "lvcreate --name '%s' --size '%s' --addtag '%s' '%s' >&p " % (
                    name,size,owner,vg)
            execute_command(comm)
            self.logger.success("Storage %s successfuly created for %s instance" %
                                (name,owner))
            self.all_lv = self.__all_logical_volume()
            return True
        else :
            self.logger.critical("Storage %s alrady in use" % name )
            return False
    ...
    def __all_physical_volume(self):
        command = "pvdisplay -C --units b --aligned --separator '|' \
                  --noheadings -o pv_name,pv_size,pv_used,pv_free,vg_name 2>/dev/null"
    ...
    def __all_logical_volume(self):
        command = "lvdisplay -C --aligned --units b --separator '|' \
                  --noheadings -o lv_name,vg_name,lv_size,lv_path 2>/dev/null"
        pvs = execute_command(command)
        result = {}
        mb = 1024*1024
        for pv in pvs.split('\n'):
